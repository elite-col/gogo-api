...
const anime_info_url = 'https://anitaku.pe/category/';
...
const Referer = 'https://playtaku.online';
const goload_stream_url = 'https://https://gotaku1.com/.com/streaming.php';
  ...
  try {
    ...
    $('div.last_episodes.loaddub > ul > li').each((i, el) => {
      list.push({
        ...
        name: $(el).find('p.name > a').attr('title').replace('"', "'"),
...
export const scrapeRecentlyAdded = async ({list = [], page = 1}) => {
  try {
    const RecentlyAdded = await axios.get(`${BASE_URL}/home.html?page=${page}`);
...
export const scrapeOngoingSeries = async ({list = [], page = 1}) => {
  try {
    const OngoingSeries = await axios.get(`${BASE_URL}/home.html?page=${page}`);
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    $('div.anime_info_body_bg > p.type:nth-child(7) > a').each((i, elem) => {
